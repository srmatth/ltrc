lnlklhd <- function(beta, gamma, X, y, t, delta, K = 3, knots = NULL, boundary_knots = NULL,
  ...
  f_midpoints_2 <- matrix(to_int_2(midpoints), ncol = K + 4, byrow = FALSE)
  ...
  if (diag_only) {
    diag_inform <- matrix(0, nrow = p + p_gamma, ncol = p + p_gamma)
    diag(diag_inform) <- diag(inform)
    # diag_inform[1:p, 1:p] <- -inform_1
    # diag_inform[(p+1):(p+p_gamma), (p+1):(p+p_gamma)] <- -inform_4
    inform <- diag_inform
  }
