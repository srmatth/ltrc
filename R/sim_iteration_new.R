...
sim_iteration_new <- function(i, FINAL_KNOTS, true_beta = c(1, 1), n, error_dist, sd_multiplier = 3,
                                  n_start = 20, type_data = "ltrc", verbose = FALSE, diag_only = TRUE, return_all = FALSE,
                                  C_UB = 44, sample_size_mult = 16) {
  ...
  x[,1] <- runif(sample_size_mult*n, min = -3, max = 3)
  x[,2] <- rbinom(sample_size_mult*n, 1, 0.5)
  ...
  t <- runif(sample_size_mult*n, 0.25, C_UB)
  ...
  mean(delta_obs)
  1 - length(obs_indx) / (sample_size_mult * n)
  ...
  tmp_dat <- list(
    n_obs = n,
    pct_truncated = 1 - length(obs_indx) / (sample_size_mult*n),
